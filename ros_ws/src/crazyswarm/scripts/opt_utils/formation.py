...
def Hc(drone_pos, fov_radius, focal_length=0.04, sigma=0.3, R=30, kappa=0.5):
    ...
    val = quadpy.disk.integrate(
        lambda p: (fpers(p[0], a1, b1, c1) * fres(p[0], a2, b2, c2)),
        [x, y],
        r,
...
def Ho(drone1_pos, drone2_pos, fov1_radius, fov2_radius, focal_length=0.04, sigma=0.3, R=30, kappa=0.5):
    ...
    val1 = quadpy.disk.integrate(
        lambda x: (fpers(x[0], a1, b1, c1) * fres(x[0], a2, b2, c2)),
        [mid_x, mid_y],
        overlap,
        quadpy.disk.Lether(6),
    )
    val2 = quadpy.disk.integrate(
        lambda x: (fpers(x[0], a3, b3, c3) * fres(x[0], a4, b4, c4)),
        [mid_x, mid_y],
        overlap,
        quadpy.disk.Lether(6),
    )
